...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true && isset($_SESSION['user_id'])) {
    ...
    try {
        $stmt = $pdo->prepare("SELECT * FROM participations p  INNER JOIN covoiturages c ON p.covoiturage_id = c.covoiturage_id WHERE p.voyageur_id = :user_id  ORDER BY p.date_depart DESC");
        $stmt->bindParam(':user_id', $userId);
        $stmt->execute();
        $historiqueCovoiturages = $stmt->fetchAll(PDO::FETCH_OBJ);
    } catch (PDOException $e) {
        $error = "Erreur lors de la récupération de l'historique des covoiturages : " . $e->getMessage();
    }
    try {
        $stmt = $pdo->prepare("
            SELECT 
...
if (isset($_POST['logout'])) {
    ...
    header("Location: https://eco-ride.online/login"); // Redirige vers la page home
    exit();
}
    ...
    if (array_key_exists($ext, $allowed) && in_array($filetype, $allowed) && $filesize <= (5 * 1024 * 1024)) { // Exemple de validation
        ...
        if (!move_uploaded_file($_FILES["photo_profil"]["tmp_name"], $photo_profil_path)) {
            ...
            try {
                ...
                $stmt->bindParam(':photo_profil', $new_filename);
                $stmt->bindParam(':id', $userId);
                $stmt->execute();
...
if (isset($_POST['ajouter_vehicule'])) {
    ...
    try {
        ...
        header("Location: https://eco-ride.online/public/account.php");
        exit();
        // Récupérer les informations utilisateur mises à jour
...
if (isset($_POST['demarrer_trajet'])) {
    // Vérification du token CSRF
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        die("Impossible de charger la page, le formulaire est invalide");
    }
    ...
    try {
        $stmt = $pdo->prepare("UPDATE participations SET statut = 'en_cours' WHERE covoiturage_id = :covoiturage_id");
        $stmt->bindParam(':covoiturage_id', $covoiturageId);
        $stmt->execute();
    } catch (PDOException $e) {
        $error = "Erreur lors de la mise à jour du statut de la participation : " . $e->getMessage();
    }
}
...
if (isset($_POST['terminer_trajet'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("UPDATE participations SET statut = 'terminer' WHERE covoiturage_id = :covoiturage_id");
        $stmt->bindParam(':covoiturage_id', $covoiturageId);
        $stmt->execute();
    } catch (PDOException $e) {
        exit("Erreur lors de la mise à jour de la participation : " . $e->getMessage());
    }
